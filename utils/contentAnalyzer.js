import analyzeTikTokData from "./tiktokAnalyzer.js";
...
export default function analyzeContent(posts = [], { platformFilter = null } = {}) {
  const filteredPosts = Array.isArray(posts)
    ? posts.filter((post) => !platformFilter || post.platform === platformFilter)
    : [];
  ...
  safePosts.forEach((post) => {
    ...
    if (!parts) return;
    const hourEntry = hoursMap.get(parts.hour) || { hour: parts.hour, likes: 0, posts: 0 };
    hourEntry.likes += safeNumber(post.likes);
    hourEntry.posts += 1;
    hoursMap.set(parts.hour, hourEntry);

    const dayEntry = daysMap.get(parts.weekday) || { weekday: WEEKDAYS[parts.weekday], likes: 0, posts: 0 };
  ...
  const topPostsByEngagement = [...safePosts]
    .map((post) => {
  ...
  if (primaryPlatform === "tiktok" && safePosts.length) {
    const tiktokInsights = analyzeTikTokData(transformForTikTok(safePosts));
