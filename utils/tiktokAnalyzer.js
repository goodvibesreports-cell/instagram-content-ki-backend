...
const DEFAULT_ANALYSIS = {
  bestTimes: { hours: Array.from({ length: 24 }, (_, hour) => ({ hour, posts: 0, avgLikes: 0, score: 0 })), bestHour: null },
  bestDays: {
    days: WEEKDAYS.map((weekday, index) => ({ weekday, index, posts: 0, avgLikes: 0, score: 0 })),
    bestDay: null
  },
  virality: { viralVideos: [], viralScore: 0, lifecycle: { averageLikesPerDay: 0, fastestGrower: null } },
  sounds: { topSounds: [], soundInsights: [] },
  themes: { keywordMap: {}, dominantThemes: [] },
  stats: { minLikes: 0, maxLikes: 0, avgLikes: 0, medianLikes: 0, totalPosts: 0 },
  creatorDNA: { mood: "Neutral", tone: "Unbekannt", narrativeStyle: "Unbekannt", contentPatterns: [], postingBehavior: "Keine Daten" }
};
...
function analyzeBestTimes(posts) {
  const buckets = Array.from({ length: 24 }, (_, hour) => ({ hour, likes: 0, posts: 0 }));
  posts.forEach((post) => {
...
function analyzeBestDays(posts) {
  const buckets = WEEKDAYS.map((weekday, index) => ({ weekday, index, likes: 0, posts: 0 }));
  posts.forEach((post) => {
    const parts = getDateParts(post.timestamp || post.date);
    if (!parts) return;
    const likes = safeNumber(post.likes);
...
function computeVirality(posts, stats) {
  if (!posts.length) {
    return { ...DEFAULT_ANALYSIS.virality };
  }
  ...
  const videos = posts
    ...
    .filter(Boolean)
    .sort((a, b) => b.multiplier - a.multiplier || b.likes - a.likes)
    .slice(0, 10);
...
function analyzeSounds(posts, stats) {
  if (!posts.length) {
    return { ...DEFAULT_ANALYSIS.sounds };
  }
  const baseline = stats.medianLikes || 1;
  const map = new Map();
  posts.forEach((post) => {
    ...
    const likes = safeNumber(post.likes);
    const entry = map.get(key) || { sound: key, likes: [], totalLikes: 0, count: 0, multiplierSum: 0 };
    entry.count += 1;
  ...
  const topSounds = [...map.values()]
    ...
    .sort((a, b) => b.viralMultiplier - a.viralMultiplier || b.usage - a.usage)
    .slice(0, 5);
...
function analyzeThemes(posts, stats = {}) {
  if (!posts.length) {
    return { ...DEFAULT_ANALYSIS.themes };
  }
  const baseline = stats.medianLikes || 1;
  const map = new Map();
  posts.forEach((post) => {
    ...
    keywords.forEach((keyword) => {
      const entry = map.get(keyword) || { keyword, count: 0, likes: 0 };
      entry.count += 1;
      entry.likes += likes;
  ...
  const dominantThemes = Object.entries(keywordMap)
    .map(([keyword, data]) => ({ keyword, ...data }))
    .sort((a, b) => b.viralMultiplier - a.viralMultiplier || b.occurrences - a.occurrences)
    .slice(0, 5);
...
function deriveNarrativeStyle(posts) {
  ...
  const avgLength =
    posts.reduce((sum, post) => sum + (post.title ? post.title.split(/\s+/).length : 0), 0) / posts.length || 0;
...
function deriveContentPatterns(dominantThemes, sounds, bestTimes) {
  ...
  if (sounds.topSounds.length) {
    patterns.push(
      `Top Sound "${sounds.topSounds[0].sound}" liefert ${sounds.topSounds[0].viralMultiplier}x Median-Likes`
    );
  }
...
export default function analyzeTikTokData(payload = [], metaOverride = {}) {
  let posts = payload;
  ...
  if (!normalizedPosts.length) {
    return { ...DEFAULT_ANALYSIS, stats: { ...DEFAULT_ANALYSIS.stats, totalPosts: 0 } };
  }
...
function resolveVideoParts(video) {
  ...
  if (typeof video.hourOfDay === "number" && typeof video.dayOfWeek === "number") {
    return { hour: video.hourOfDay, weekday: video.dayOfWeek };
  }
...
export function analyzeTikTokVideos(videos = []) {
  ...
  const bestPostingHours = hourBuckets
  ...
  const postingDaysOfWeek = dayBuckets
    .filter((bucket) => bucket.videoCount > 0)
    .map((bucket) => ({
  ...
  const topVideos = [...safeVideos]
    .sort((a, b) => safeNumber(b.likes) - safeNumber(a.likes))
    .slice(0, 10);
