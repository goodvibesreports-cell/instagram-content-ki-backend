...
function detectTikTokDataType(json) {
  ...
  const deletedList =
    get(json, ["Post", "Recently Deleted Posts", "PostList"]) ||
    get(json, ["Post", "Recently Deleted Videos", "VideoList"]);
  const watchHistory = get(json, ["Your Activity", "Watch History", "VideoList"]);
  const likeList =
    get(json, ["Your Activity", "Like List", "ItemFavoriteList"]) ||
    get(json, ["Activity", "Like List", "ItemFavoriteList"]);
...
export function detectPlatform(json, sourceFileName = "unknown") {
  const result = {
    platform: "unknown",
  ...
  if (tikTokScore >= 2 || dataType !== PLATFORM_DATA_TYPES.UNKNOWN) {
    ...
    result.reason =
      dataType === PLATFORM_DATA_TYPES.POSTS
        ? "Found Post.Posts.VideoList and TikTok-typical root keys"
        : `Detected TikTok export type: ${dataType}`;
    return result;
  }
...
export const detectPlatformAndType = detectPlatform;
