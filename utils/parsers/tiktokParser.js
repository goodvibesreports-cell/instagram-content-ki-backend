...
exports.parseTiktokJson = parseTiktokJson;
...
function normalizeDate(value) {
  ...
  if (!str) return null;
  let candidate = str;
  if (/^\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}$/.test(str) && !str.toLowerCase().includes("gmt") && !/[+-]\d{2}:?\d{2}$/.test(str) && !str.endsWith("z")) {
...
function buildNormalizedPost(item = {}, {
  isDeleted = false
} = {}) {
  ...
  const title = cleanText(item.Title) || cleanText(item.Caption) || cleanText(item.Description) || cleanText(item.AlternateText) || "TikTok Video";
  ...
  const comments = COMMENT_FIELDS.reduce((current, field) => current !== 0 ? current : normalizeNumber(item[field]), 0);
...
function parseTiktokJson(payload) {
  const posts = [];
  ...
  SECTION_DEFINITIONS.forEach(({
    ...
    section.forEach(item => {
      const post = buildNormalizedPost(item, {
        isDeleted
      });
      if (!post) {
        return;
