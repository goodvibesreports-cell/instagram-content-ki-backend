...
function extractTextFromData(data = []) {
  ...
  return data
    .map((entry) => entry?.post || entry?.text || entry?.title || "")
    .filter(Boolean)
    .join("\n")
    .trim();
}
...
function normalizeFacebookItem(item) {
  const isoDate =
    toIsoDate(item.timestamp) || toIsoDate(item.published_time) || toIsoDate(item.created_time) || toIsoDate(item.date);
  ...
  const link =
    ...
    (Array.isArray(item.attachments)
      ? item.attachments.find((attachment) => attachment?.url || attachment?.href)?.url
      : "") ||
    "";
