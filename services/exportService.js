...
export async function generateInsightsPdf({
  ...
  return await new Promise((resolve, reject) => {
    ...
    doc.fontSize(20).fillColor("#111827").text("TikTok Performance Report", { align: "center" });
    doc.moveDown(0.3);
    doc.fontSize(12).fillColor("#6b7280").text(`Erstellt am ${now}`, { align: "center" });
    doc.moveDown();

    doc.fontSize(12).fillColor("#374151").text(`Creator: ${profileName}`, { continued: false });
    ...
    const summaryRows = [
      ...
      ["Average Likes", stats.avgLikes ? Math.round(stats.avgLikes) : "-"],
      ["Median Likes", stats.medianLikes ? Math.round(stats.medianLikes) : "-"],
      ["Best Hour", analysis.bestTimes?.bestHour ?? "-"],
      ["Best Day", analysis.bestDays?.bestDay ?? "-"]
    ];
    summaryRows.forEach(([label, value]) => {
      doc.fontSize(12).fillColor("#111827").text(`${label}: `, { continued: true });
    ...
    if (chartBuffers.some((chart) => chart.buffer)) {
      ...
      chartBuffers.forEach((chart) => {
        if (chart.buffer) {
          ...
          doc.moveDown(0.2);
          doc.image(chart.buffer, { fit: [480, 220], align: "center" });
          doc.moveDown(0.6);
        }
      });
    }
    ...
    const dnaRows = [
      ...
      ["Mood", creatorDNA.mood || "-"],
      ["Narrative Style", creatorDNA.narrativeStyle || "-"],
      ["Posting Behavior", creatorDNA.postingBehavior || "-"]
    ];
    ...
    videoImages.forEach((video, index) => {
      ...
      if (video.imageBuffer) {
        doc.moveDown(0.2);
        doc.image(video.imageBuffer, { fit: [460, 200], align: "center" });
      }
      doc.moveDown(0.8);
    });
    ...
    if (aiSummary) {
      ...
      doc.moveDown(0.4);
      const items = [
        ["Best Time Strategy", aiSummary.bestTimeStrategy],
        ["Content Style", aiSummary.contentStyle],
        ["Hook Type", aiSummary.hookType],
        ["Posting Frequency", aiSummary.postingFrequency],
        ["What To Stop Doing", aiSummary.stopDoing]
      ];
...
export function generateInsightsCsv(posts = []) {
  const header = [
    "date",
    "link",
    "likes",
  ...
  const rows = posts.map((post) => {
    ...
    const dayPosted = parsedDate
      ? parsedDate.toLocaleDateString("de-DE", { weekday: "long" })
      : "";
    ...
    return [
      ...
      post.link || "",
      likes,
      (post.caption || post.title || "").replace(/\r?\n/g, " "),
      post.sound || "",
      post.location || "",
      hourPosted,
      dayPosted,
